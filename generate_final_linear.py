...
def generate_final_linear():
    ...
    # Calculate Offset
    # Target: "Prophet's Journey to Taif" (Content, not ToC lines)
    # Exclude lines with dots "...." or digits at end (ToC indicators)
    target = "Journey to Taif"
    offset = 4 # Default based on observation (14->10)
    ...
    for i in range(min(50, len(linear_pages))):
        ...
        if target.upper() in text.upper():
            ...
            if ".........." not in linear_pages[i] and not re.search(r'\.\s+\d+$', text):
                print(f"Found Content Anchor '{target}' at Logical Page {i}")
                # ToC says 15.
                offset = i - 15
    ...
    print(f"ToC covered books: {used_books}")
    for bid, meta in BOOKS_META.items():
